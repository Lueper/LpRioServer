...
bool LpSocketCore::CloseHandle(HANDLE handle) {
	if (handle == nullptr)
		return false;
...
void LpSocketCore::DeregisterRioBuffer(RIO_EXTENSION_FUNCTION_TABLE& rio, RIO_BUFFERID bufferId) {
	return rio.RIODeregisterBuffer(bufferId);
}
